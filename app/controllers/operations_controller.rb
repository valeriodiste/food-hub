class OperationsController < ApplicationController
	...
	def find_random_recipe()
		response = HTTP.timeout(10).get("https://api.spoonacular.com/recipes/random", :params => {:apiKey => Rails.application.credentials[:spoonocular][:api_key], :number => '3'})
	...
	def request_api(url)
		response = HTTP.timeout(10).get(url, :params => {:apiKey => Rails.application.credentials[:spoonocular][:api_key], :addRecipeInformation => 'true', :number => '6'})
		return nil if response.status != 200
	...
	def search_diet_type(diet_type)
		response = HTTP.timeout(10).get("https://api.spoonacular.com/recipes/complexSearch?diet=#{diet_type}", :params => {:apiKey => Rails.application.credentials[:spoonocular][:api_key], :addRecipeInformation => 'true', :number => '6'})
		return nil if response.status != 200
		response.parse['results']
	end
